...
export default function Sidebar({ open, setOpen }) {
  ...
  const navigation = [
    ...
    { name: 'Payments', href: '/payments', icon: 'currency-dollar' },
  ...
  return (
    <div className={`${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static inset-y-0 left-0 w-64 bg-indigo-700 text-white transition-transform duration-200 ease-in-out z-30`}>
      ...
      <nav className="p-4">
        <ul className="space-y-2">
          {navigation.map((item) => (
            <li key={item.name}>
              <Link href={item.href}>
                <a className={`flex items-center p-3 rounded-lg ${router.pathname === item.href ? 'bg-indigo-800' : 'hover:bg-indigo-600'}`}>
